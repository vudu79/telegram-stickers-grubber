...
def load(browsers, use_local_file=True, verify_ssl=True):
    ...
    if use_local_file:
        try:
            json_lines = (
                ilr.files("fake_useragent.data").joinpath("browsers.files").read_text()
            )
            ...
            data = {}
            logger.warning(
                "Could not find local data/files file or could not parse the contents. Fallback to external resource.",
                exc_info=exc,
            )
        else:
            fetch_online = False
